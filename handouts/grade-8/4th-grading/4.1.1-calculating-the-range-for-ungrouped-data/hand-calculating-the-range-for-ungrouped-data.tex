 \input{../../../handout-preamble-2023}
\input{../../../../common-preamble-2023}
\usepackage[paperwidth=0.5\paperwidth,paperheight=0.5\paperheight, left=4.5mm, right=6mm, top=2mm, bottom=2mm]{geometry}
\pgfpagesuselayout{4 on 1}[letterpaper, border shrink=0mm]
   
              
\begin{document}
\fontsize{11}{12.5}\selectfont

\insertwatermark
    
%\begin{multicols*}{2} 			
\input{vs-calculating-the-range-for-ungrouped-data-input}
\input{ps-calculating-the-range-for-ungrouped-data-input1}
\input{ps-calculating-the-range-for-ungrouped-data-input2}	
%\end{multicols*} 
  
\newpage 
\insertwatermark

%\begin{multicols*}{2} 			
\input{vs-calculating-the-range-for-ungrouped-data-input}
\input{ps-calculating-the-range-for-ungrouped-data-input1}
\input{ps-calculating-the-range-for-ungrouped-data-input2}	
%\end{multicols*} 

\newpage 
\insertwatermark

%\begin{multicols*}{2} 			
\input{vs-calculating-the-range-for-ungrouped-data-input}
\input{ps-calculating-the-range-for-ungrouped-data-input1}
\input{ps-calculating-the-range-for-ungrouped-data-input2}	
%\end{multicols*} 

\newpage 
\insertwatermark

%\begin{multicols*}{2} 			
\input{vs-calculating-the-range-for-ungrouped-data-input}
\input{ps-calculating-the-range-for-ungrouped-data-input1}
\input{ps-calculating-the-range-for-ungrouped-data-input2}	
%\end{multicols*} 
\end{document}
